...
.claude/             Lean GSD agents, commands, workflows
```
...
This project uses **Lean GSD** — 8 commands, 4 agents.

```
/start   →  Question → research → product design → roadmap
/plan    →  Research (optional) → planner agent → PLAN.md
/build   →  Wave-based parallel subagents with atomic commits
...
```
/handoff  →  Save session state to specs/handoffs/
/pickup   →  Load most recent handoff, suggest next steps
/prime    →  Orient Claude in any project (structure, stack, key files)
```
